...
if hObject.Value
    ...
    stim = square(2*pi*str2double(gd.Controls.puffFreq.String)*t);
    stim(stim<0)=0;
    
    % Create Triggers
    ITI = str2double(gd.Parameters.ITI.String);
    trial = zeros(round(gd.Experiment.params.samplingFrequency*(dur+ITI)),1);
    trial(1:numel(stim)) = stim;
    ...
    DAQ.addDigitalChannel('Dev1',gd.Experiment.stim.setup.Port{1},'OutputOnly');
    
    % Send out triggers
    while hObject.Value
        DAQ.QueueOutputScan(trial);
        DAQ.StartForeground;
    end
    clear DAQ
...
if hObject.Value
    ...
    % Create Stim
    dur = str2double(gd.Parameters.stimDur.String);
    stim = wgn(dur,1,str2double(gd.Controls.soundPower.String));
    ...
    DAQ = daq.createSession('ni');                      % initialize session
    DAQ.Rate = gd.Experiment.params.samplingFrequency;  % set sampling frequencywhile hObject.Value
    DAQ.addAnalogOutputChannel('Dev1',str2double(gd.Experiment.stim.setup.Port{3}),'Voltage')
