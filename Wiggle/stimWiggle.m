function [TrialInfo, SaveFile] = stimWiggle(SaveFile, varargin)
...
gd.Experiment.stim.setup = table(...
    ...
    gd.Internal.DAQ.ID([1,1,1,1,2])',...
    'VariableNames',{'Name','Port','Active','DAQ'});
gd.Experiment.stim.pistonCombinations = {};
gd.Internal.daq = []; % Place holder for offline control of stimulus

gd.Internal.viewHandle = []; % for viewing triggers
...
gd.Stimuli.ports = uitable(...
    ...
    'Units',                'Normalized',...
    'Position',             [0,.4,.6,.6],...
    'ColumnName',           {'Name','Port','Active','DAQ'},...
...
% stimulus frequency
gd.Stimuli2.frequency = uicontrol(...
    'Style',                'edit',...
    'Parent',               gd.Stimuli2.panel,...
...
gd.Stimuli2.frequencyText = uicontrol(...
    'Style',                'text',...
    'Parent',               gd.Stimuli2.panel,...
    'String',               'Frequency',...
...
gd.Stimuli2.voltage = uicontrol(...
    ...
    'Callback',             @(hObject,eventdata)ChangeStim(hObject, eventdata, guidata(hObject)));
gd.Stimuli2.voltageText = uicontrol(...
    'Style',                'text',...
    'Parent',               gd.Stimuli2.panel,...
    'String',               'Voltage',...
    'HorizontalAlignment',  'right',...
    ...
    'Position',             [.5,.4,.35,.1]);
% wail duration
gd.Stimuli2.wailDuration = uicontrol(...
    'Style',                'edit',...
    'Parent',               gd.Stimuli2.panel,...
    ...
    'Callback',             @(hObject,eventdata)ChangeStim(hObject, eventdata, guidata(hObject)));
gd.Stimuli2.wailDurationText = uicontrol(...
    'Style',                'text',...
    'Parent',               gd.Stimuli2.panel,...
...
function [DAQ,Active] = initDAQ(hStimTable,Fs)
...
elseif eventdata.Indices(2)==4
    % ADD: check that newly input device name exists
end
end
...
gd.Experiment.timing.ITI = str2double(gd.Parameters.ITI.String);
end
...
if get(hObject,'Value')
    [DAQ,ActivePiezos] = initDAQ(gd.Stimuli.ports,gd.Experiment.params.samplingFrequency);
    if ~any(ActivePiezos)
        clear DAQ
...
%         Experiment.PistonTrigger(startTrig:endTrig) = Triggers;
        ...
        if Experiment.params.whiskerTracking
            ...
            if ~gd.Parameters.wtType.Value	% single trigger per frame
                ...
                Experiment.Triggers(WTstartTrig, strcmp(OutChannels,'O_WhiskerTracker')) = 1; % image during stimulus period
            end
        end
        ...
        currentTrial = 0;
        TrialInfo = struct('StimID', [], 'numRandomScansPost', []);
