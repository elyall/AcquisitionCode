...
gd.Controls.wtType = uicontrol(...
    'Style',                'togglebutton',...
    'String',               'Trigger Per Frame',...
    'Parent',               gd.Controls.panel,...
    'Units',                'normalized',...
    'Position',             [.3,.4,.3,.1],...
    'UserData',             {[.94,.94,.94;1,1,1],'Trigger Per Frame','Trigger Per Trial'},...
    'Callback',             @(hObject,eventdata)set(hObject,'BackgroundColor',hObject.UserData{1}(hObject.Value+1,:),'String',hObject.UserData{hObject.Value+2}));
...
%         DAQ.addClockConnection('External',['Dev1/' clkTerminal],'ScanClock');
        ...
        Experiment.ImagingType = gd.Saving.imagingType.String{gd.Saving.imagingType.Value};
